...
contract AcrossSMA {
    ...
    modifier allowedInputToken(address _token) {
        require(IAcrossSMAFactory(factory).inputTokenAllowed(_token), "Input token not allowed");
        _;
    }

    modifier allowedOutputToken(address _token) {
        require(IAcrossSMAFactory(factory).outputTokenAllowed(_token), "Output token not allowed");
        _;
    }

    modifier allowedTargetChain(uint256 _chainId) {
        require(IAcrossSMAFactory(factory).targetChainMulticallHandler(_chainId) != address(0), "Target chain not allowed");
        _;
    }

    // TODO: Accept AaveSMA params in constructor
    constructor(address _pSymmAddress, address _spokePoolAddress, address _factory, bytes32 _custodyId) {
    ...
    ) external allowedInputToken(_inputToken) allowedOutputToken(_outputToken) allowedTargetChain(_destinationChainId) onlyPSymm {
        // deposit amount of token
        IERC20(_inputToken).approve(address(spokePool), _amount);

        spokePool.depositV3(
            address(this), // address depositor,
            IAcrossSMAFactory(factory).targetChainMulticallHandler(_destinationChainId), // address recipient,
