...
contract UniswapV3SMA {
    ...
    modifier allowedToken(address _token) {
        require(IUniswapV3SMAFactory(factory).tokenAllowed(_token), "Token not allowed");
        _;
    }

    modifier allowedFee(uint24 _fee) {
        require(IUniswapV3SMAFactory(factory).feeAllowed(_fee), "Fee not allowed");
        _;
    }

    constructor(address _pSymmAddress, address _routerAddress, address _factory, bytes32 _custodyId) {
    ...
    function swapExactInputSingle(
        ...
        uint24 fee,
        uint256 amountIn
    ) external allowedToken(tokenIn) allowedToken(tokenOut) allowedFee(fee) onlyPSymm returns (uint256 amountOut) {
    ...
    function swapExactInput(
        ...
        require(IUniswapV3SMAFactory(factory).tokenAllowed(tokenIn), "Input token not allowed");
        ...
        require(IUniswapV3SMAFactory(factory).tokenAllowed(tokenOut), "Output token not allowed");
    ...
    ) external allowedToken(tokenIn) allowedToken(tokenOut) allowedFee(fee) onlyPSymm returns (uint256 amountIn) {
        // Approve router to spend tokens
    ...
    function extractTokenIn(bytes calldata path) internal pure returns (address) {
        require(path.length >= 20, "Invalid path");
        return address(uint160(bytes20(path[:20])));
    }
    
    // Helper to extract output token from path
    function extractTokenOut(bytes calldata path) internal pure returns (address) {
        require(path.length >= 20, "Invalid path");
