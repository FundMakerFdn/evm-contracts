...
contract UniswapV3SMAFactory is IUniswapV3SMAFactory {
    event UniswapV3SMADeployed(address indexed smaAddress, bytes32 indexed custodyId);
    ...
    // Implement the ISMAFactory.deploySMA interface for PSYMM
    ...
    function setSlippageLimitBps(uint256 _slippageLimitBps) external {
        require(_slippageLimitBps <= 10000, "Slippage limit cannot exceed 100%");
        slippageLimitBps = _slippageLimitBps;
    }
    
    function setMaxDeadlineExtension(uint256 _maxDeadlineExtension) external {
        require(_maxDeadlineExtension <= 24 hours, "Max deadline extension cannot exceed 24 hours");
