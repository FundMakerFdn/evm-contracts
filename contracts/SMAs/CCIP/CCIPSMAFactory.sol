...
contract CCIPSMAFactory is ICCIPSMAFactory, Ownable {
    ...
    event CCIPSMADeployed(address indexed smaAddress, bytes32 indexed custodyId, address localCCIPReceiver);
    ...
    constructor(address _pSymmAddress, address _ccipContractAddress, address _localCCIPReceiverAddress) Ownable(msg.sender) {
        require(_pSymmAddress != address(0), "Factory: Invalid pSymm address");
        require(_ccipContractAddress != address(0), "Factory: Invalid CCIP contract address");
        require(_localCCIPReceiverAddress != address(0), "Factory: Invalid local CCIPReceiver address");
    ...
    function setLocalCCIPReceiver(address _newLocalCCIPReceiverAddress) external onlyOwner {
        require(_newLocalCCIPReceiverAddress != address(0), "Factory: Invalid new local CCIPReceiver address");
        localCCIPReceiverAddress = _newLocalCCIPReceiverAddress;
        emit LocalCCIPReceiverUpdated(_newLocalCCIPReceiverAddress);
    }
    
    function destinationChainAllowed(uint64 chainSelector) external view override returns (bool) {
        return destinationChains[chainSelector];
    }
    
    function setDestinationChain(uint64 _chainSelector, bool _allowed) external onlyOwner {
        destinationChains[_chainSelector] = _allowed;
        emit DestinationChainUpdated(_chainSelector, _allowed);
    }
    
    function deploySMA(bytes calldata data) external override returns (address) {
        require(msg.sender == address(pSymm), "Factory: Only pSymm can deploy SMAs");
        
        bytes32 custodyId = abi.decode(data, (bytes32));
