...
contract CCIPReceiver is IAny2EVMMessageReceiver, Ownable {
    ...
    event LocalDestinationWhitelisted(
        address indexed destinationSMA,
        bool status
    );
    ...
    function setLocalDestinationWhitelist(
        address _localCCIPSMAAddress,
        bool _status
    ) external onlyOwner {
        require(
            _localCCIPSMAAddress != address(0),
            "CCIPReceiver: Invalid local CCIPSMA address"
        );
    ...
    function ccipReceive(
        ...
        CCIPSMA.CCIPMessage memory decodedCCIPMessage = abi.decode(
            message.data,
            (CCIPSMA.CCIPMessage)
        );
        ...
        require(
            whitelistedLocalDestinations[targetLocalSMA],
            "CCIPReceiver: Target local SMA not whitelisted"
        );

        (bool success, ) = targetLocalSMA.call(
            abi.encodeWithSelector(
                CCIPSMA.handleCCIPMessage.selector,
                message.data
            )
        );
        require(
            success,
            "CCIPReceiver: Failed to forward message to target SMA"
        );
