...
import "../SMAs/UniswapV3/IUniswapV3Router.sol";
...
contract MockUniswapV3Router is IUniswapV3Router {
    ...
    event ExactInputSingleExecuted(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut);
    event ExactInputExecuted(bytes path, uint256 amountIn, uint256 amountOut);
    event ExactOutputSingleExecuted(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut);
    ...
    function exactInputSingle(ExactInputSingleParams calldata params) external payable override returns (uint256 amountOut) {
        ...
        emit ExactInputSingleExecuted(params.tokenIn, params.tokenOut, params.amountIn, amountOut);
    ...
    function exactInput(ExactInputParams calldata params) external payable override returns (uint256 amountOut) {
        ...
        require(params.path.length >= 40, "Path must include at least two tokens");
        ...
        uint24 fee = uint24(uint8(params.path[20])) * 65536 + uint24(uint8(params.path[21])) * 256 + uint24(uint8(params.path[22]));
    ...
    function exactOutputSingle(ExactOutputSingleParams calldata params) external payable override returns (uint256 amountIn) {
        ...
        // Apply the fee
        amountIn = amountInBeforeFee + (amountInBeforeFee * uint256(params.fee)) / (1_000_000 - uint256(params.fee));
        
        // Ensure maximum input amount is not exceeded
        require(amountIn <= params.amountInMaximum, "Excessive input amount required");
        ...
        emit ExactOutputSingleExecuted(params.tokenIn, params.tokenOut, amountIn, params.amountOut);
    ...
    function exactOutput(ExactOutputParams calldata params) external payable override returns (uint256 amountIn) {
        ...
        // Apply the fee
        amountIn = amountInBeforeFee + (amountInBeforeFee * uint256(fee)) / (1_000_000 - uint256(fee));
