...
describe("ETF Operations", function () {
  ...
  beforeEach(async function () {
    ...
    await subject.usdc.connect(subject.owner).approve(
      await subject.psymm.getAddress(),
      ethers.parseEther("10")
    );
    await subject.psymm.connect(subject.owner).addressToCustody(
      custodyId,
    ...
    const data = ethers.AbiCoder.defaultAbiCoder().encode(
      ["address", "string", "string", "bytes32", "address", "uint256", "uint256", "uint256", "uint256", "uint256", "uint256"],
      [
    ...
    // Deploy SMA/Index
    const tx = await subject.psymm.connect(subject.owner).deploySMA(
      "index",
  ...
  describe("Index Creation", function () {
    ...
    it("Should register index in registry", async function () {
      let isRegistered;
      try {
        isRegistered = await indexRegistry.isIndexRegistered(await index.getAddress());
      } catch (e) {
