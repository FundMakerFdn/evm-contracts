import {
  keccak256,
  toHex,
  parseAbiParameters,
  encodeAbiParameters,
  concat,
  pad,
  Address,
  Hex,
} from "viem";
import { StandardMerkleTree } from "@openzeppelin/merkle-tree";

// Define types for the PPM items
type PPMItemType = "deploySMA" | "callSMA" | "custodyToAddress" | "custodyToSMA" | "changeCustodyState";

interface Party {
  parity: number;
  x: Hex;
}

interface PPMItemBase {
  type: PPMItemType;
  chainId: number | number[];
  pSymm: Address;
  state: number | number[];
  args: any; // Will be converted to encoded bytes
  party: Party | Party[];
}

interface PPMItemExpanded {
  type: PPMItemType;
  chainId: number;
  pSymm: Address;
  state: number;
  args: Hex;
  party: Party;
}

interface DeploySMAArgs {
  smaType: string;
  factoryAddress: Address;
  callData: Hex;
}

interface CallSMAArgs {
  smaType: string;
  smaAddress: Address;
  callData: {
    type: string;
    args: any[];
  } | Hex;
}

interface CustodyToAddressArgs {
  receiver: Address;
}

interface CustodyToSMAArgs {
  smaType: string;
  token: Address;
}

interface ChangeCustodyStateArgs {
  newState: number;
}

type PPMArgs = DeploySMAArgs | CallSMAArgs | CustodyToAddressArgs | CustodyToSMAArgs | ChangeCustodyStateArgs;

interface PPMItem extends PPMItemBase {
  args: PPMArgs;
}

class PPMBuilder {
  private ppmItems: PPMItemExpanded[];
  private argsToTypes: Record<PPMItemType, string>;

  constructor() {
    this.ppmItems = [];
    this.argsToTypes = {
      deploySMA: "string smaType,address factoryAddress,bytes callData",
      callSMA: "string smaType,address smaAddress,bytes callData",
      custodyToAddress: "address receiver",
      custodyToSMA: "string smaType,address token",
      changeCustodyState: "uint8 newState",
    };
  }

  // Expands object combinations for array values in party, chainId, and state
  private expandObject(obj: PPMItemBase): PPMItemExpanded[] {
    const keysToExpand = ["party", "chainId", "state"] as const;

    const arrays: Record<string, any[]> = Object.fromEntries(
      keysToExpand
        .filter((key) => key in obj)
        .map((key) => [key, Array.isArray(obj[key]) ? obj[key] : [obj[key]]])
    );

    if (Object.keys(arrays).length === 0) return [obj as unknown as PPMItemExpanded];

    const combinations = Object.entries(arrays).reduce(
      (acc, [key, values]) => {
        return acc.flatMap((combo) =>
          values.map((value) => ({ ...combo, [key]: value }))
        );
      },
      [{}]
    );

    const baseObj = { ...obj };
    keysToExpand.forEach((key) => delete baseObj[key]);

    return combinations.map((combo) => ({ ...baseObj, ...combo } as PPMItemExpanded));
  }

  addItem(_item: PPMItem): PPMItemExpanded[] {
    const item = { ..._item }; // copy by value
    
    if (item.type === "callSMA" && typeof (item.args as CallSMAArgs).callData !== "string") {
      const callSMAArgs = item.args as CallSMAArgs;
      const callDataObj = callSMAArgs.callData as { type: string; args: any[] };
      const callData = this.encodeCalldata(
        callDataObj.type,
        callDataObj.args
      );
      callSMAArgs.callData = callData;
    }
    
    const parsed = parseAbiParameters(this.argsToTypes[item.type]).slice(
      0,
      Object.keys(item.args).length
    );

    const argList: any[] = [];
    for (const { name } of parsed) {
      if (name) {
        argList.push((item.args as any)[name]);
      }
    }

    item.args = encodeAbiParameters(parsed, argList) as any;
    console.log("Full encoded args:", item.args);

    const expanded = this.expandObject(item);
    this.ppmItems.push(...expanded);
    return expanded;
  }

  getPPM(): PPMItemExpanded[] {
    return this.ppmItems;
  }

  buildTreeRoot(): Hex {
    const values = this.ppmItems.map((item) => [
      item.type,
      item.chainId,
      item.pSymm,
      item.state,
      item.args,
      item.party.parity,
      pad(item.party.x),
    ]);
    console.log("Merkle tree leaves: ", values);

    const tree = StandardMerkleTree.of(values, [
      "string", // entry type
      "uint256", // chainId
      "address", // pSymm
      "uint8", // state
      "bytes", // abi.encode(args)
      "uint8", // party.parity
      "bytes32", // party.x
    ]);

    return tree.root;
  }

  encodeCalldata(funcType: string, funcArgs: any[]): Hex {
    // funcType example: "borrow(address,uint256)"
    // Compute the function selector (first 4 bytes of keccak256 of the function signature)
    const selector = keccak256(toHex(funcType)).slice(0, 10); // selector is 0x01020304

    // Extract parameter types from funcType
    const paramTypes = funcType.slice(
      funcType.indexOf("(") + 1,
      funcType.lastIndexOf(")")
    );

    const params = encodeAbiParameters(
      // support partial calldata in funcArgs
      parseAbiParameters(paramTypes).slice(0, funcArgs.length),
      funcArgs
    );

    const callData = concat([selector, params]);
    console.log("Encoded callData:", callData);

    return callData;
  }
}

export { PPMBuilder };
export type { PPMItem, PPMItemExpanded, Party }; 