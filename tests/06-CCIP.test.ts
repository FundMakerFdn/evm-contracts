...
describe("CCIP End-to-End Integration", function () {
  ...
  const MOCK_CCIP_MESSAGE_ID = ethers.keccak256(ethers.toUtf8Bytes("mock-ccip-message-id"));

  async function deployFullCCIPFixture() {
    const [owner, psymmOwner, ccipSmaOwner, user1, otherUser] = await ethers.getSigners();

    const MockERC20Factory = await ethers.getContractFactory("MockERC20");
    const linkToken = await MockERC20Factory.deploy("ChainLink", "LINK", 18);
    await linkToken.waitForDeployment();

    const MockCCIPRouterFactory = await ethers.getContractFactory("MockCCIPRouter");
    ...
    const updatePPMActionLeafInnerHashPreImage = abiCoder.encode(
        ...
        ["updatePPM", chainId, psymmAddress, stateForLeaf, encodedEmptyArgs, pubKeyParityForLeaf, pubKeyXForLeaf]
    );
    const updatePPMActionLeafInnerHash = ethers.keccak256(updatePPMActionLeafInnerHashPreImage);
    CUSTODY_ID_UNDER_TEST = ethers.keccak256(ethers.solidityPacked(["bytes32"], [updatePPMActionLeafInnerHash]));

    await psymm.connect(psymmOwner).addressToCustody(CUSTODY_ID_UNDER_TEST, await linkToken.getAddress(), 0);

    const ccipSourceFactory = await ethers.getContractFactory("CCIP");
    const ccipSource = await ccipSourceFactory.deploy(await mockRouterContract.getAddress());
    await ccipSource.waitForDeployment();

    const ccipReceiverSourceFactory = await ethers.getContractFactory("CCIPReceiver");
    const ccipReceiverSource = await ccipReceiverSourceFactory.deploy(await mockRouterContract.getAddress());
    await ccipReceiverSource.waitForDeployment();

    const ccipSMAFactorySourceFactory = await ethers.getContractFactory("CCIPSMAFactory");
    ...
    const ccipDest = await ccipDestFactory.deploy(await mockRouterContract.getAddress());
    await ccipDest.waitForDeployment();

    const ccipReceiverDestFactory = await ethers.getContractFactory("CCIPReceiver");
    const ccipReceiverDest = await ccipReceiverDestFactory.deploy(await mockRouterContract.getAddress());
    await ccipReceiverDest.waitForDeployment();

    const ccipSMAFactoryDestFactory = await ethers.getContractFactory("CCIPSMAFactory");
    ...
    await ccipSource.setDestinationCCIPReceiver(CHAIN_SELECTOR_DEST, await ccipReceiverDest.getAddress());

    await ccipReceiverDest.setSourceSenderWhitelist(CHAIN_SELECTOR_SRC, await ccipSource.getAddress(), true);
    await ccipReceiverDest.setLocalDestinationWhitelist(await ccipSMADest.getAddress(), true);
    ...
    await linkToken.connect(ccipSmaOwner).approve(await ccipSource.getAddress(), MaxUint256);
    
    await mockRouterContract.setFee(MOCK_FEE_AMOUNT);
    await mockRouterContract.setMessageId(MOCK_CCIP_MESSAGE_ID);

    return {
      owner, psymmOwner, ccipSmaOwner, user1, otherUser,
      linkToken, mockRouter: mockRouterContract,
      psymm,
      ccipSource, ccipReceiverSource, ccipSMAFactorySource,
      ccipDest, ccipReceiverDest, ccipSMAFactoryDest, ccipSMADest,
  ...
  describe("CCIPSMAFactory Configuration", function() {
    it("Should set local CCIPReceiver address correctly", async function() {
      const { ccipSMAFactorySource, ccipReceiverSource } = await loadFixture(deployFullCCIPFixture);
      expect(await ccipSMAFactorySource.localCCIPReceiverAddress()).to.equal(await ccipReceiverSource.getAddress());
    });

    it("Should allow owner to update local CCIPReceiver address", async function() {
      const { owner, ccipSMAFactorySource, otherUser } = await loadFixture(deployFullCCIPFixture);
      await ccipSMAFactorySource.connect(owner).setLocalCCIPReceiver(otherUser.address);
      expect(await ccipSMAFactorySource.localCCIPReceiverAddress()).to.equal(otherUser.address);
    });
  });
  ...
  describe("Sending and Receiving UpdatePPM Message via CCIP", function () {
    it("Should allow a whitelisted caller on CCIPSource to send an UpdatePPM message that is processed by CCIPSMADest", async function () {
      ...
      const ccipMessagePayloadArray = [
        ...
        abiCoder.encode(["bytes32", "(bytes32,uint8,uint256,(uint8,bytes32),(bytes32,bytes32),bytes32[])"], [NEW_PPM_ROOT, verificationDataArray])
      ];
      const encodedCCIPMessage = abiCoder.encode(["(uint8,address,bytes)"], [ccipMessagePayloadArray]);


      const psymmOnDestination = psymm;
      const initialPPMValue = await psymmOnDestination.getPPM(CUSTODY_ID_FOR_TESTING);
      expect(initialPPMValue).to.equal(CUSTODY_ID_FOR_TESTING, "Initial PPM value on destination PSYMM should be the custody ID itself.");

      // --- Send the CCIP message from source ---
      await expect(ccipSource.connect(ccipSmaOwner).sendMessage(
        CHAIN_SELECTOR_DEST,
        encodedCCIPMessage,
      ...
      await network.provider.send("hardhat_impersonateAccount", [mockRouterAddress]);
      await network.provider.send("hardhat_setBalance", [mockRouterAddress, ethers.toBeHex(ethers.parseEther("1"))]); 
      ...
      await expect(ccipReceiverDest.connect(mockRouterSigner).ccipReceive(any2EvmMessage))
        .to.emit(ccipSMADest, "UpdatePPMMessageReceived") // Assuming CCIPSMA emits this
        .withArgs(CUSTODY_ID_FOR_TESTING, NEW_PPM_ROOT, verificationDataObj.timestamp);
      
      await network.provider.send("hardhat_stopImpersonatingAccount", [mockRouterAddress]);

      const finalPPMValue = await psymmOnDestination.getPPM(CUSTODY_ID_FOR_TESTING);
      expect(finalPPMValue).to.equal(NEW_PPM_ROOT, "Final PPM value on destination PSYMM should be updated to NEW_PPM_ROOT after CCIP processing.");
    });

    it("Should revert if a non-whitelisted source CCIP contract tries to send a message", async function () {
      const { mockRouter, ccipReceiverDest, otherUser } = await loadFixture(deployFullCCIPFixture);
      ...
      await expect(ccipReceiverDest.connect(mockRouterSigner).ccipReceive(any2EvmMessage))
        .to.be.rejectedWith("CCIPReceiver: Source CCIP contract not whitelisted");
      
      await network.provider.send("hardhat_stopImpersonatingAccount", [mockRouterAddress]);
    });

    it("Should revert if message targets a non-whitelisted local SMA", async function () {
      const { mockRouter, ccipSource, ccipReceiverDest, otherUser } = await loadFixture(deployFullCCIPFixture);
